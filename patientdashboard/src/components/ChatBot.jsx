...
const Chatbot = ({ token }) => {
  ...
  const [messages, setMessages] = useState([
    { user: 'AI', text: 'Hi, how can I help you today?' }
  ]);
    ...
    <div>
      ...
      <div style={{ boxShadow: '0 0 #0000, 0 0 #0000, 0 1px 2px 0 rgb(0 0 0 / 0.05)' }}
        ...
        <div className="flex flex-col space-y-1.5 pb-6">
          ...
          <p className="text-sm text-[#6b7280] leading-3">Powered by +medfit</p>
        </div>
          ...
          {messages.map((message, index) => (
            <div key={index} className="flex gap-3 my-4 text-gray-600 text-sm flex-1">
              <span className="relative flex shrink-0 overflow-hidden rounded-full w-8 h-8">
                <div className={`rounded-full bg-gray-100 border p-1 ${message.user === 'AI' ? 'text-black' : 'text-white'}`}>
              ...
              <p className="leading-relaxed">
                <span className="block font-bold text-gray-700">{message.user}</span> {message.text}
              </p>
            </div>
          ))}
        </div>
